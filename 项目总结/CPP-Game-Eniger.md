...
``` py
def pso(num_particles, num_dimensions, max_iter, inertia_weight):
    # 初始化粒子群
    personal_acceleration = 2
    social_acceleration = 2
    positions = torch.randint(1, 40, (num_particles, num_dimensions)).float()
    velocities = torch.rand(num_particles, num_dimensions)
    def objective_func(position):
        return torch.sum(lenth * density * position)
    fitnesses = torch.tensor([objective_func(position) for position in positions])
    particles = [Particle(position, velocity, fitness) for position, velocity, fitness in
                 zip(positions, velocities, fitnesses)]

    # 初始化全局最优解
    global_best_position = positions[torch.argmin(fitnesses)]
    global_best_fitness = torch.min(fitnesses)
    ...
    for i in range(max_iter):
        for particle in particles:
            # 更新粒子速度和位置
            r1 = torch.rand(num_dimensions)
            r2 = torch.rand(num_dimensions)
            cognitive_velocity = personal_acceleration * r1 * (particle.best_position - particle.position)
            social_velocity = social_acceleration * r2 * (global_best_position - particle.position)
            particle.velocity = inertia_weight * particle.velocity + cognitive_velocity + social_velocity
            particle.position += particle.velocity
            particle.position = torch.clamp(particle.position, 1, 50)


            # 更新粒子个体最优解
            particle_fitness = objective_func(particle.position)
            print()
            if (particle_fitness < particle.best_fitness) and ((bar10(particle.position.tolist())[0]) == 0):
                particle.best_position = particle.position
                particle.best_fitness = particle_fitness
                print(particle.best_position)

            # 更新全局最优解
            if (particle_fitness < global_best_fitness) and ((bar10(particle.position.tolist())[0]) == 0):
                global_best_fitness = particle_fitness
                global_best_position = particle.position
    if ((bar10(global_best_position)[0]) == 1):
